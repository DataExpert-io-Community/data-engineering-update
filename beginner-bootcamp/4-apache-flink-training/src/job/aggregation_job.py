...
def log_aggregation():
    ...
    try:
        ...
        # Create Postgres sinks (session-based tables)
        aggregated_table = "processed_sessions_aggregated"
        sink_ddl1 = f"""
        ...
        """
        t_env.execute_sql(sink_ddl1)

        aggregated_sink_table = "processed_sessions_aggregated_source"
        sink_ddl2 = f"""
            CREATE TABLE {aggregated_sink_table} (
                session_start TIMESTAMP(3),
                session_end TIMESTAMP(3),
                host VARCHAR,
                referrer VARCHAR,
                ...
                'connector' = 'jdbc',
                'url' = '{os.environ.get("POSTGRES_URL")}',
                'table-name' = '{aggregated_sink_table}',
                'username' = '{os.environ.get("POSTGRES_USER", "postgres")}',
                'password' = '{os.environ.get("POSTGRES_PASSWORD", "postgres")}',
                'driver' = 'org.postgresql.Driver'
            );
        """
        t_env.execute_sql(sink_ddl2)

        # Aggregate by session + host
        ...
        # Aggregate by session + host + referrer
        t_env.from_path(source_table) \
            .window(
                Session.with_gap(lit(5).minutes).on(col("window_timestamp")).alias("w")
            ).group_by(
